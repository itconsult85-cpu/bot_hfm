...
<div class="mb-4 mt-2">
    ...
    <p class="text-muted small">Pantau performa pendaftaran dan aktivitas grup VIP Anda.</p>
</div>
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
    ...
    <div class="d-flex flex-column flex-md-row gap-2">
        <input type="text" id="searchInput" class="form-control rounded-pill px-4" placeholder="Cari Nama atau ID HFM..." style="min-width: 250px;">
...
<div class="row g-3" id="memberContainer">
    <?php foreach ($members as $m): ?>
        <div class="col-12 member-item" data-nama="<?= strtolower(esc($m['nama'] ?? '')) ?>" data-idhfm="<?= esc($m['id_hfm']) ?>" data-nowa="<?= esc($m['no_wa']) ?>">
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow-lg">
            ...
            <form action="<?= base_url('AdminDashboard/update-member') ?>" method="POST">
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label small fw-semibold text-muted">ID HFM (Bisa diedit)</label>
                        <!-- Input tersembunyi untuk menyimpan ID lama sebagai patokan saat update -->
...
<script>
    // KITA HAPUS DEKLARASI APP_ROUTES AGAR TIDAK BENTROK DENGAN FILE TEMPLATE
    ...
    document.querySelectorAll('.btn-hapus').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            // Langsung gunakan base_url di sini
    ...
    async function syncData(btn, id_hfm) {
        try {
            ...
            console.log("Mencoba mengakses URL:", targetUrl);

            const response = await fetch(targetUrl);

            if (!response.ok) {
                alert("Server Error! HTTP Status: " + response.status + "\n(URL mungkin salah atau server down)");
                throw new Error("HTTP Error " + response.status);
            }

            const resText = await response.text();
            console.log("Jawaban Server:", resText);

            let res;
            try {
                res = JSON.parse(resText);
            } catch (e) {
                alert("Sistem Error!\nServer tidak mengembalikan data JSON.\n\nLihat Console (F12) untuk detailnya.");
                console.error("Data bukan JSON! Isi data yang diterima:", resText);
            ...
            if (res.status === 'sukses') {
                ...
                document.getElementById('depo-' + id_hfm).innerText = depoFormatted;
                ...
                if (rawTrade && rawTrade !== '0000-00-00 00:00:00' && rawTrade !== 'N/A') {
                    if (rawTrade.includes('-')) {
                        ...
                        document.getElementById('trade-' + id_hfm).innerHTML = `<span class="badge bg-info-subtle text-info border border-info-subtle rounded-pill fw-semibold small"><i class="bi bi-graph-up me-1"></i>${tgl}</span>`;
                    }
                } else {
                    document.getElementById('trade-' + id_hfm).innerHTML = `<span class="badge bg-light border text-muted rounded-pill small">Belum Trading</span>`;
                }
                document.getElementById('status-' + id_hfm).innerHTML = '<span class="badge bg-success-subtle text-success border border-success-subtle rounded-pill px-3 py-2 small fw-semibold">Aktif</span>';
            } else if (res.status === 'lepas_ib') {
                document.getElementById('status-' + id_hfm).innerHTML = '<span class="badge bg-danger-subtle text-danger border border-danger-subtle rounded-pill px-3 py-2 small fw-semibold">Lepas IB</span>';
            } else {
    ...
    async function syncSemua() {
        ...
        const tombolCek = document.querySelectorAll('.btn-sync-single');

        if (tombolCek.length === 0) {
        ...
        for (let i = 0; i < tombolCek.length; i++) {
            let btn = tombolCek[i];
            btn.click();
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function() {
                ...
                filteredItems = allItems.filter(item => {
                    ...
                    return nama.includes(keyword) || idhfm.includes(keyword) || nowa.includes(keyword);
                });
                renderPage(1);
            });
        }
